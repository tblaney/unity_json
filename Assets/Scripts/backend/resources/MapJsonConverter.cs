    ...
    public class MapJsonConverter : JsonConverter
    {
            ...
            try
            {
                switch (value.Type)
                {
                    ...
                    case JTokenType.Date:
                        map.Set<DateTime>(name, value.ToObject<DateTime>());
                        break;
                    case JTokenType.Object:
                        ...
                        {
                            LOG.Console($"Failed to parse nested object for key: {name}. Error: {ex.Message}");
                        }
                        break;
                    case JTokenType.Array:
                        ...
                        {
                            LOG.Console($"Failed to handle array for key: {name}. Error: {ex.Message}");
                        }
                        break;
                    default:
                        LOG.Console($"Unsupported JSON token type: {value.Type} for key: {name}");
                        break;
                }
            }
            catch (Exception ex)
            {
                LOG.Console($"Failed processing key: {name} with value type: {value.Type}. Error: {ex.Message}");
            }
        }
            ...
            JsonSerializer serializer)
        {
            ...
            foreach (KeyValuePair<string, object> kvp in dictionary)
            {
                ...
                {
                    writer.WriteValue(dateTimeValue.ToString("o")); // Write the DateTime in ISO 8601 format
                }
        ...
        private Dictionary<string, object> MapToDictionary(Map map)
        {
            ...
            if (map.Elements != null)
            {
                foreach (string elementKey in map.Elements.Keys)
                {
                    ...
                    else if (element is DateTime dateTimeAttr)
                    {
                        result[elementKey] = dateTimeAttr;
                    }
