    ...
    public class CastTicker : Ticker
    {
        ...
        void ProcessRequest(string name)
        {
            ...
            if (args.Get<bool>("is_origin_local", false))
            {
                ...
                Vec v = (Node.Point.Position).Add(Node.Point.North.Multiply(originCurrent[2])).Add(Node.Point.East.Multiply(originCurrent[0]));
                v = v.Add(new Vec(0,originCurrent[1],0));

                args.Set<Bag<float>>("origin", new Bag<float>(v.x, v.y, v.z));
            }
