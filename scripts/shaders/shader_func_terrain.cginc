...
uniform float _TerrainHoleSize = 1.0;
...
float3 CorrectVertForHoles(float3 worldPosition, float3 vertPosition)
{
    float2 uvW = worldPosition.xz - _Position.xz;
    uvW = uvW / (_OrthographicCamSize * 2);
    uvW += 0.5;			
    float3 waterRingVisibility = tex2Dlod(_TerrainHoleTex, float4(uvW, 0, 0));

    return vertPosition - float3(0,_TerrainHoleSize*step(0.5, waterRingVisibility.r),0);
}
float3 CorrectColorForHoles(float3 worldPosition, float3 colorIn, float3 colorHole)
{
    float2 uvW = worldPosition.xz - _Position.xz;
    uvW = uvW / (_OrthographicCamSize * 2);
    uvW += 0.5;			
    ...
    return colorIn*(1-step(0.5, waterRingVisibility.r)) + colorHole*step(0.5, waterRingVisibility.r);
}
...
void SplatmapVert(inout appdata_full v, out Input data)
{
    ...
    v.vertex.xyz = CorrectVertForSnow(localPosition, worldPosition, v.normal);
    v.vertex.xyz = CorrectVertForHoles(worldPosition, v.vertex.xyz);
    ...
    data.localPos = localPosition;
    data.tc.xy = v.texcoord.xy;
    data.worldNormal = v.normal.xyz;
...
#endif
{
    ...
    float2 uvSplat0 = TRANSFORM_TEX(IN.tc.xy* _Splat0_ST.xy + _Splat0_ST.zw, _Splat0);
    float2 uvSplat1 = TRANSFORM_TEX(IN.tc.xy* _Splat1_ST.xy + _Splat1_ST.zw, _Splat1);
    float2 uvSplat2 = TRANSFORM_TEX(IN.tc.xy* _Splat2_ST.xy + _Splat2_ST.zw, _Splat2);
    float2 uvSplat3 = TRANSFORM_TEX(IN.tc.xy* _Splat3_ST.xy + _Splat3_ST.zw, _Splat3);
